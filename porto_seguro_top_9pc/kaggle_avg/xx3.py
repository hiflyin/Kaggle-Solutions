...
id_pairs = [([b for b in range(10) if b not in a], list(a)) for a in list(itertools.combinations(range(10), 2))]
...
for i in range(len(chunk_pairs)):
    ...
    y_train, y_valid = y.iloc[pair[0]].copy(), y.iloc[pair[1]].copy()
    X_train, X_valid = X.iloc[pair[0], :].copy(), X.iloc[pair[1], :].copy()
    X_test = test_df.copy()
    ...
    y_valid_pred.iloc[pair[1]] += pred

    # Accumulate test set predictions
    y_test_pred += pred_test

np.save("val_xx3.npy", y_valid_pred.values/9)
np.save("test_xx3.npy", y_test_pred/len(chunk_pairs))
